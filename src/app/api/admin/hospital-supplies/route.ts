...
// GET /api/admin/hospital-supplies - 列表（带分页）
export async function GET(request: NextRequest) {
  ...
  const { results } = await db
    .prepare('SELECT * FROM hospital_supplies ORDER BY created_at DESC LIMIT ? OFFSET ?')
    .bind(limit, offset)
    .all();
  const { results: countRow } = await db
    .prepare('SELECT COUNT(*) as total FROM hospital_supplies')
    .all();
...
// PATCH /api/admin/hospital-supplies?id=1 - 更新核实状态
...
// DELETE /api/admin/hospital-supplies?id=1
