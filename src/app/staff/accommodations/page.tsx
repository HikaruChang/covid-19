...
export default function StaffAccommodationsPage() {
  ...
  const renderItem = useCallback((item: any) => (
    <Card key={item.id ?? item.name} sx={{ borderTop: '4px solid #a20002', position: 'relative' }}>
      ...
      <Box sx={{ position: 'absolute', top: 8, right: 8, textAlign: 'right', fontSize: '0.75rem', color: 'text.secondary' }}>
        {item.beds && (
          <Box><Typography variant="h6" component="span" fontWeight={900} color="text.primary">{item.beds}</Typography> {t('accommodations.bedsLeft')}</Box>
        )}
        {item.room && (
          <Box><Typography variant="h6" component="span" fontWeight={900} color="text.primary">{item.room}</Typography> {t('accommodations.roomsLeft')}</Box>
        )}
        {item.distance != null && (
      ...
      <CardContent>
        <Typography variant="subtitle2">
          ...
          <br />{t('accommodations.address')}：{item.address}
        </Typography>
        {item.notes && (
          <Typography variant="subtitle2" color="error" sx={{ mt: 0.5 }}>
            {t('accommodations.notes')}：{item.notes}
          </Typography>
        )}
      </CardContent>
      <Divider />
  ...
  if (loading && !data.length) {
    return (
      <Box sx={{ mx: 1 }}>
        <Typography variant="h5" fontWeight={700} gutterBottom>{t('accommodations.title')}</Typography>
    ...
    <Box sx={{ mx: 1 }}>
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', mx: 1, mb: 1 }}>
        <FormControlLabel
          ...
          label={t('accommodations.filterAvailable')}
        />
        <FormControlLabel
          control={<Checkbox checked={filterLinBao} onChange={(_, v) => setFilterLinBao(v)} />}
          label={t('accommodations.filterNoLinen')}
        />
      </Box>
