...
export default function AdminHospitalSuppliesPage() {
  const [rows, setRows] = useState<HospitalSupply[]>([]);
  ...
  const fetchData = async (p: number) => {
    ...
    setError('');
    try {
      const res = await fetch(`/api/admin/hospital-supplies?page=${p + 1}`);
  ...
  const handleToggleVerify = async (row: HospitalSupply) => {
    await fetch(`/api/admin/hospital-supplies?id=${row.id}`, {
      method: 'PATCH',
  ...
  const handleDelete = async (id: number) => {
    ...
    await fetch(`/api/admin/hospital-supplies?id=${id}`, { method: 'DELETE' });
    fetchData(page);
  };
